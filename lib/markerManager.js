...
export function initMapMarkers(map){
  rollingSales.slice(1000,1100).forEach(home => {
    createHomeSaleMarker(home, map);
  })
};

export function initStationMarkers(map) {
  mta.forEach(station => {
    createStationMarker(station, map);
  })
};

function createHomeSaleMarker(home, map) {
...
function createStationMarker(station, map) {
  // const image = "https://s3.amazonaws.com/safehavns-dev/mark.png";
